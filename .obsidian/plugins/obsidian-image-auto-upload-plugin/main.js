...
isStream$1.transform = (stream) =>
  isStream$1.duplex(stream) && typeof stream._transform === "function";
...
class StringType {
  ...
  get(uint8Array, offset) {
    return node_buffer.Buffer.from(uint8Array).toString(
      this.encoding,
      offset,
      offset + this.len,
    );
  ...
  async parse(tokenizer) {
    ...
    if (this.checkString("%PDF")) {
      try {
        ...
        const buffer = node_buffer.Buffer.alloc(
        ...
        while (children > 0) {
          ...
          if (element.id === 0x42_82) {
            const rawValue = await tokenizer.readToken(
              new StringType(element.len, "utf-8"),
            );
...
async function downloadAllImageFiles(plugin) {
  ...
  for (const file of fileArray) {
    ...
    let name = decodeURI(pathBrowserify.parse(asset).name).replaceAll(
      /[\\\\/:*?\"<>|]/g,
      "-",
    );
    ...
    if (response.ok) {
      ...
      imageArray.push({
        source: file.source,
        name: name,
        path: obsidian.normalizePath(
          pathBrowserify.relative(
            obsidian.normalizePath(activeFolder),
            obsidian.normalizePath(response.path),
          ),
        ),
      });
    }
...
async function download(plugin, url, folderPath, name) {
  ...
  try {
    let path = obsidian.normalizePath(
      pathBrowserify.join(folderPath, `${name}.${type.ext}`),
    );
    // 如果文件名已存在，则用随机值替换，不对文件后缀进行判断
    if (await plugin.app.vault.adapter.exists(path)) {
      path = obsidian.normalizePath(
        pathBrowserify.join(folderPath, `${uuid()}.${type.ext}`),
      );
    }
    ...
    for (const match of fileArray) {
      ...
      if (encodedUri.startsWith("http")) {
        ...
        if (
          ...
          const filePath = pathBrowserify.resolve(
            pathBrowserify.join(
              basePath,
              pathBrowserify.dirname(activeFile.path),
            ),
            decodeURI(encodedUri),
          );
          if (require$$0.existsSync(filePath)) {
            const path = obsidian.normalizePath(
              pathBrowserify.relative(
                obsidian.normalizePath(basePath),
                obsidian.normalizePath(
                  pathBrowserify.resolve(
                    pathBrowserify.join(
                      basePath,
                      pathBrowserify.dirname(activeFile.path),
                    ),
