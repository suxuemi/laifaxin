...
> 客户列表越来越长，管理变得一团糟？别担心，**标签 (Tags)** 和 **视图 (Views)** 就是帮你化繁为简的两大神器。
...
> **简单来说：** **1️⃣标签** 告诉你：“**他是谁？**”， **2️⃣视图** 帮你实现：“**要找谁？**”

掌握这两个工具，你就能把客户管理得井井有条，从而将更多宝贵精力，聚焦于**拿下订单**。

---

> 我们用一张图，了解 `标签`、`视图`和`邮件序列`是如何协同工作实现客户管理与主动营销的：
...
graph LR
    ...
    subgraph sg3 ["第三步：营销 (用序列)"]
        direction TB
        D[fa:fa-cogs ④ 将筛出的客户<br><i>加入邮件序列或群发</i>];
    end
...
## 🏷️ 一、玩转客户标签 (为客户精准画像) {#mastering-contact-tags}

> 一个好的标签，应该像一张内容丰富的名片，让你**一眼就能看懂客户！**

:::info 核心方法
- **标签**的作用是描述客户的静态属性，比如：他是哪个国家的？做的什么产品？
...
- **推荐公式**：<mark>语言 - 国家 - 产品/行业 - 供应链角色</mark>
...
_图示：在联系人列表中新建标签，在标签下拉菜单中点击“新建”_

![设置标签名称、所属目录、标签颜色等信息](https://cos.files.maozhishi.com/data/web/web-files/img/20250818102849.png)
_图示：在联系人列表中新建标签，设置标签信息：名称、目录、颜色等信息_
...
## 👀 二、用好联系人视图 (智能筛选目标客户) {#using-contact-views}

:::info 核心价值
- 如果说标签是**静态的“名片”**，那么 **<span style={{color: '#8e44ad'}}>视图</span>** 就是**动态的“筛选器”**。
- 它最大的用途是帮你 **快速找到一批人**，或者在群发时 **快速排除一批人**。一次设置，永久复用！
:::
...
## 🚀 三、实战演练 (把理论用起来) {#practical-application}
...
3.  **第三步：设置邮件序列**
    - 参照教程[📚 邮件序列](./email-sequence-guide) ，进入 [🌍邮件序列](https://web.laifaxin.com/marketing/sequences) 功能页面，新建一个序列。
