...
如果这正是你的写照，那么本指南会是你的破局之道。我们手把手带你搭建一个<span style={{color: '#e67c00'}}>**能自动运转的客户开发系统**</span>，将混乱转为生产力。
:::
...
graph TD
    ...
    subgraph SG_RETAIN ["✅ 被留下 (可营销)"]
        E["<b>需营销客群</b><br><i>(未命中任何排除规则)</i>"]
    end

    B -- "❌ 规则命中" --> SG_EXCLUDE;
    B -- "✅ 规则未命中" --> SG_RETAIN;

    F["fa:fa-rocket **第三步: 启动自动化营销**<br><i>对待营销客群执行</i>"]
...
graph TD
    ...
    %% 定义流程
    ViewAll -- "🌟添加收件人🌟" --> SeqAwaken
    SeqAwaken -- "<b>客户行为:</b><br>打开邮件" --> ViewRead
    ViewRead -- "🌟添加收件人🌟" --> SeqNurture
