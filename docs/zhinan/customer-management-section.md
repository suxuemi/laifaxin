...
| :--------------------------------- | :------------------------------------------------------------------------------------------- |
| **① 🗂️ 分类的黑洞**<br /><i>邮件发不对人</i> | 把精心准备的德语邮件发给美国客户，或把A产品资料发给只对B感兴趣的买家。（<span style={{color: '#e74c3c'}}>**时间白费，机会错失**</span>）  |
| **② 🧠 记忆的黑洞**<br /><i>跟进状态混乱</i>  | 反复骚扰已成交的老客户，或再次联系已明确拒绝的联系人。（<span style={{color: '#e74c3c'}}>**专业形象受损，客户关系告急**</span>）       |
| **③ 💎 机会的黑洞**<br /><i>优质客户流失</i>  | 频繁打开邮件的“热”客户，因未得到及时、更高频的跟进，慢慢变“冷”。（<span style={{color: '#e74c3c'}}>**最大订单机会，眼睁睁溜走**</span>） |
...
工具是基础，但清晰的策略才是关键。下面这张**客户转化路径图**，就是你未来工作的核心流程：
...
- **✅ 人机协作，无缝衔接：** 当系统收到“询盘”时，自动化流程会自动停止，你可一对一跟进，确保每个机会都能被牢牢抓住。
...
现在让我们按照以下三大任务的顺序进行操作，你将亲手构建一个属于自己的、高效运转的客户开发系统。
...
**1. 创建标签分组 (建立标签体系)**
- 进入[**标签管理**](https://web.laifaxin.com/settings/tags) 页面，按照 [📚标签分组：客户、意向、风险](./contacts-tags-views#tag-groups)进行新建。
...
**🎯 任务目标：** 参照[📚新建视图](./contacts-tags-views#creating-and-managing-views)在[联系人](https://web.laifaxin.com/contacts/contacts)页面创建动态**筛选视图** ，让系统能自动识别“应联系谁”和“不应打扰谁”。
...
```mermaid
---
title: 客户安全筛选与营销自动化流程
---
graph TD
    A["fa:fa-users **第一步: 准备客户池**<br><i>你的所有客户</i>"] --> B{"fa:fa-filter **第二步: 营销排除标签**<br><i>基于标签自动排除</i>"};

    subgraph SG_EXCLUDE ["⬇️ 被筛掉 (不营销)"]
        direction TB
        C["
            <b>❌ 风险客户</b><br>
            避免骚扰与浪费
    ...
    subgraph SG_RETAIN ["✅ 被留下 (可营销)"]
        E["<b>安全客群</b><br><i>(未命中任何排除规则)</i>"]
    end

    B -- "❌ 规则命中" --> SG_EXCLUDE;
    B -- "✅ 规则未命中" --> SG_RETAIN;

    F["fa:fa-rocket **第三步: 启动自动化营销**<br><i>对安全客群执行</i>"]
    
    SG_RETAIN -- "下一步: 执行营销" --> F;
    ...
    %% Subgraph Styling
    style SG_EXCLUDE fill:#fff5f5,stroke:#e53e3e,stroke-dasharray: 5 5
    style SG_RETAIN fill:#f0fff4,stroke:#38a169,stroke-dasharray: 5 5
    
    %% Node Styling inside Subgraphs
    style C fill:#f8d7da,stroke:#721c24,stroke-width:2px
    style D fill:#fff3cd,stroke:#856404,stroke-width:2px
    style E fill:#d4edda,stroke:#155724
    
    %% Final Action Node Styling
    style F fill:#cce5ff,stroke:#004085,stroke-width:3px
```
...
graph LR
    ...
    subgraph "有阅读客户（7~10天/轮）"
        C("<b>[Read] 英语-皮筏艇-经销商</b><br><i>基于阅读行为筛选</i>")
    end
...
```
- `[All]` 视图客户被加入**唤醒序列**，当客户**阅读邮件**会进入 `[Read]` 视图，加入高频**培育序列**，实现客户分层与加速跟进。
...
graph LR
    ...
    %% Styling
    style A fill:#e6f2ff,stroke:#004085
    style B fill:#fffbe6,stroke:#856404
    style C fill:#e6ffed,stroke:#155724
...
![设置了4个步骤，每个步骤3个模板](https://cos.files.maozhishi.com/data/web/web-files/img/20250818194719.png)
...
![开启序列](https://cos.files.maozhishi.com/data/web/web-files/img/20250818195614.png)
